...
as an app tile, favicon, avatar, and wordmark lockup. **"F Monogram"** was
selected: a blocky, geometric letterform built from three solid rectangles
plus a lime locator dot — the most abstract and brandable of the four, and
the one that holds up best at tiny favicon sizes.
...
The outer tile stays sharp (`0` radius) to match the rest of the UI's
